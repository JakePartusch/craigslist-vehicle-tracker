...
var debug = (0, _debug2.default)('craigslist'),
    ...
    QUERY_KEYS = ['category', 'maxAsk', 'minAsk', 'query', 'autoMakeModel', 'minAutoYear', 'maxAutoYear'],
    QUERY_PARAM_MAX = '&max_price=',
    QUERY_PARAM_MIN = '&min_price=',
    QUERY_PARAM_QUERY = '&query=',
    QUERY_AUTO_MAKE_MODEL = '&auto_make_model=',
    QUERY_MIN_AUTO_YEAR = '&min_auto_year=',
    QUERY_MAX_AUTO_YEAR = '&max_auto_year=',
...
function _getPostings(options, markup) {
	...
	$('div.content').find('li.result-row').each(function (i, element) {
		...
		posting = {
			...
			hasPic: ($(element).find('span.result-tags').text() || '').trim() !== '',
			location: ($(element).find('.result-hood').text() || '').replace(/[\(,\)]/g, '') // santize
			.trim(),
			pid: ($(element).attr('data-pid') || '').trim(),
			price: ($(element).find('span.result-price').text() || '').replace(/^\&\#x0024\;/g, '').trim(), // sanitize
...
function _getRequestOptions(options, query) {
    ...
    // add max asking price (if specified)
    if (!_core2.default.Validation.isEmpty(options.autoMakeModel)) {
        requestOptions.path = [requestOptions.path, QUERY_AUTO_MAKE_MODEL, encodeURIComponent(options.autoMakeModel)].join('');
    }

    // add max asking price (if specified)
    if (!_core2.default.Validation.isEmpty(options.minAutoYear)) {
        requestOptions.path = [requestOptions.path, QUERY_MIN_AUTO_YEAR, options.minAutoYear].join('');
    }

    // add max asking price (if specified)
    if (!_core2.default.Validation.isEmpty(options.maxAutoYear)) {
        requestOptions.path = [requestOptions.path, QUERY_MAX_AUTO_YEAR, options.maxAutoYear].join('');
    }
